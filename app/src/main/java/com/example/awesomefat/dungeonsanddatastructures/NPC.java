...
public class NPC extends Thread
{
    ...
    private void wanderNPC()
    {
        Room r = this.getCurrentRoom()
        Random rand = new Random();
        int takeExit = rand(0, this.getCurrentRoom.getExits.length())
        r.takeExit(takeExit, this);

        if(this.getCurrentRoom() != takeExit)
        {
            this.getContext().update(this.getCurrentRoom());
        }
    }
